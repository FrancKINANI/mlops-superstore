...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.debug(f"Features temporelles créées: {['order_month', 'order_quarter', 'order_dayofweek']}")
...
def build_preprocessor() -> ColumnTransformer:
    ...
    logger.debug(f"Preprocessor construit avec {len(cat_features)} features catégorielles et {len(num_features)} numériques")
    return preprocessor
