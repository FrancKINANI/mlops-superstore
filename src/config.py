...
class DataConfig:
    ...
    processed_data_path: Path = Path(os.getenv("PROCESSED_DATA_PATH", "data/processed/Superstore_processed.csv"))
...
class MLFlowConfig:
    ...
    experiment_name: str = os.getenv("MLFLOW_EXPERIMENT_NAME", "superstore-profitability")
    ...
    def __post_init__(self):
        ...
        if not self.tracking_uri:
            logger.warning("MLFLOW_TRACKING_URI non défini, utilisant valeur par défaut")
...
class ModelConfig:
    ...
    preprocessor_save_path: Path = Path(os.getenv("PREPROCESSOR_SAVE_PATH", "mlartifacts/preprocessor"))
...
class APIConfig:
    ...
    model_path: str = os.getenv("MODEL_PATH", "mlartifacts/1/models/m-2acd3303576e4ab2967290e07fa3d929/artifacts/model.pkl")
...
class AirflowConfig:
    ...
    project_root: Path = Path(os.getenv("PROJECT_ROOT", "/home/franck/Documents/01_Cours/Data/IA/Projets/efm/ML/mlops-superstore"))
...
class MonitoringConfig:
    ...
    prometheus_config_path: Path = Path(os.getenv("PROMETHEUS_CONFIG_PATH", "monitoring/prometheus.yml"))
    drift_report_path: Path = Path(os.getenv("DRIFT_REPORT_PATH", "monitoring/reports/drift_report.html"))
