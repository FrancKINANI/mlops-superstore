...
def promote_to_production():
    ...
    try:
        client.set_registered_model_alias(
            name=BEST_MODEL_NAME, alias="staging", version=BEST_MODEL_VERSION
        )
        print(f"[Staging] {BEST_MODEL_NAME} v{BEST_MODEL_VERSION}")
    except Exception:
        # Fallback pour anciennes versions de MLflow
        client.transition_model_version_stage(
            name=BEST_MODEL_NAME, version=BEST_MODEL_VERSION, stage="Staging"
        )
        print(f"[Staging] {BEST_MODEL_NAME} v{BEST_MODEL_VERSION}")
    ...
    try:
        client.set_registered_model_alias(
            name=BEST_MODEL_NAME, alias="prod", version=BEST_MODEL_VERSION
        )
        print(f"[Production] {BEST_MODEL_NAME} v{BEST_MODEL_VERSION} — ACTIF")
    except Exception:
        # Fallback pour anciennes versions de MLflow
        client.transition_model_version_stage(
            name=BEST_MODEL_NAME, version=BEST_MODEL_VERSION, stage="Production"
        )
        print(f"[Production] {BEST_MODEL_NAME} v{BEST_MODEL_VERSION} — ACTIF")
